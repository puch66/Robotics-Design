...
//bool array to handle flags raised when some action has finished to perform
bool done[16];

//each element represents a servo
movement s[16];

//iterator to count the current step of an emotion
int i = 0;

//iterator to loop over some repeating action
int repeat_actions = 0;

//iterator to loop over the idle phases
int idle_step = 0;


/**************************************************
***********  MOVEMENT CLASS FUNCTIONS  ************
***************************************************/
...
//initializes internal variables for the servos and sets initial positions  
...
//sets the servo to a new position with a certain velocity
...
bool set_eyelids(int m, float v = 0.05) {
  if(m == 0) {
    ...
    s[5].new_position({S5_OPEN, v}); 
      ...
      s[0].new_position({ S0_CLOSE-10, v });
      s[1].new_position({ S1_CLOSE+10, v });
      s[4].new_position({ S4_CLOSE+10, v });
      s[5].new_position({ S5_CLOSE-10, v });
  }
...
//rotates relatively to a specific character on the table
...
//Resets the robot position of each body part to their initial value
bool reset_position() {
...
//Performs idle emotion 
bool do_idle(Characters c, long rand) {
    //idle loop is split in two parts, first one perfomrs an action
    if (idle_step == 0) {
        //select a random action to do
        if (rand < 33) {
            if (i == 0) if (set_eyelids(1, 0.2)) i++;
            if (i == 1) if (set_eyelids(2, 0.2)) i++;
            if (i == 2) if (set_eyelids(1, 0.2)) i++;
        ...
        if (rand >= 67 && rand < 83) {
            if (i == 0) {
                ...
                if (!done[1]) done[1] = s[14].wait(750);
                if (done[0] && done[1]) {
            ...
            if (i == 1) {
                if (!done[0]) done[0] = set_body_rotation(1, 0.1);
                if (!done[1]) done[1] = s[14].wait(750);
                if (done[0] && done[1]) {
                    done[0] = false; done[1] = false;
                    i++;
    ...
    //second part of the idle loop waits for a second
    if (idle_step == 1) {
        if (i == 0) if (s[14].wait(1000)) i++;
        if (i == 1) {
            i = 0;
            idle_step = 0;
            return true;
...
bool do_happy(Characters c) {
  ...
  if (i == 6 && repeat_actions < 11) {
      if (repeat_actions == 3 || repeat_actions == 7) myDFPlayer.play(3);
      repeat_actions++;
      i = 4;
  }
  if(i == 6 && repeat_actions == 11) {
...
//Performs shocked emotion
bool do_shocked(Characters c) {
    if (i == 0) if (set_body_rotation(1, 0.2, c)) i++;
    if (i == 1) if (set_tail(2)) {
        i++;
        myDFPlayer.play(5);
    }
    if (i == 2) {
...
//Performs cautious emotion (suspicious) 
...
//Reads a message coming from god
bool read_message(int number) {
    if (play_song(number)) return true;
    if (i == 0) {
        if (!done[0]) done[0] = set_mouth(1, 0.1);
        if (!done[1]) done[1] = s[14].wait(125);
        if (done[0] && done[1]) {
            done[0] = false; done[1] = false;
            i++;
        }
    ...
    if (i == 1) {
        if (!done[0]) done[0] = set_mouth(3, 0.1);
        if (!done[1]) done[1] = s[14].wait(125);
        if (done[0] && done[1]) {
            done[0] = false; done[1] = false;
            i++;
